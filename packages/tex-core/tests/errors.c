/* Error paths: every rejection is a structured error with an exact status,
 * message, and source byte range — fail-fast, never a silent skip (plan
 * decision D4, section 5.1). */

#include <string.h>

#include "harness.h"
#include "tex_core.h"

typedef struct txc_error_case {
    const char *label;
    const char *source;
    size_t length;
    tex_core_mode mode;
    tex_core_status status;
    const char *message;
    size_t begin;
    size_t end;
} txc_error_case;

static const txc_error_case TXC_ERROR_CASES[] = {
    {"lone invalid byte",
     "\xFF",
     1,
     TEX_CORE_MODE_DOCUMENT,
     TEX_CORE_STATUS_INVALID_UTF8,
     "invalid UTF-8 sequence",
     0,
     1},
    {"truncated sequence at end",
     "ab\xC3",
     3,
     TEX_CORE_MODE_DOCUMENT,
     TEX_CORE_STATUS_INVALID_UTF8,
     "invalid UTF-8 sequence",
     2,
     3},
    {"overlong encoding",
     "\xC0\x80",
     2,
     TEX_CORE_MODE_DOCUMENT,
     TEX_CORE_STATUS_INVALID_UTF8,
     "invalid UTF-8 sequence",
     0,
     2},
    {"surrogate encoding",
     "\xED\xA0\x80",
     3,
     TEX_CORE_MODE_DOCUMENT,
     TEX_CORE_STATUS_INVALID_UTF8,
     "invalid UTF-8 sequence",
     0,
     3},
    {"codepoint past U+10FFFF",
     "\xF5\x80\x80\x80",
     4,
     TEX_CORE_MODE_DOCUMENT,
     TEX_CORE_STATUS_INVALID_UTF8,
     "invalid UTF-8 sequence",
     0,
     4},
    {"stray continuation byte",
     "a\x80",
     2,
     TEX_CORE_MODE_DOCUMENT,
     TEX_CORE_STATUS_INVALID_UTF8,
     "invalid UTF-8 sequence",
     1,
     2},
    {"invalid byte in a control symbol",
     "\\\xFF",
     2,
     TEX_CORE_MODE_DOCUMENT,
     TEX_CORE_STATUS_INVALID_UTF8,
     "invalid UTF-8 sequence",
     1,
     2},

    {"math shift", "$", 1, TEX_CORE_MODE_DOCUMENT, TEX_CORE_STATUS_UNSUPPORTED, "unsupported character U+0024", 0, 1},
    {"unclosed group", "{a", 2, TEX_CORE_MODE_MATH_INLINE, TEX_CORE_STATUS_UNSUPPORTED, "unclosed group", 0, 1},
    {"unmatched closing brace",
     "a}",
     2,
     TEX_CORE_MODE_MATH_INLINE,
     TEX_CORE_STATUS_UNSUPPORTED,
     "unmatched closing brace",
     1,
     2},
    {"begin group in document mode",
     "{",
     1,
     TEX_CORE_MODE_DOCUMENT,
     TEX_CORE_STATUS_UNSUPPORTED,
     "unsupported character U+007B",
     0,
     1},
    {"end group in document mode",
     "}",
     1,
     TEX_CORE_MODE_DOCUMENT,
     TEX_CORE_STATUS_UNSUPPORTED,
     "unsupported character U+007D",
     0,
     1},
    {"comment", "a%b", 3, TEX_CORE_MODE_DOCUMENT, TEX_CORE_STATUS_UNSUPPORTED, "unsupported character U+0025", 1, 2},
    {"parameter", "#", 1, TEX_CORE_MODE_DOCUMENT, TEX_CORE_STATUS_UNSUPPORTED, "unsupported character U+0023", 0, 1},
    {"double superscript",
     "x^a^b",
     5,
     TEX_CORE_MODE_MATH_INLINE,
     TEX_CORE_STATUS_UNSUPPORTED,
     "double superscript",
     3,
     4},
    {"double subscript", "x_a_b", 5, TEX_CORE_MODE_MATH_INLINE, TEX_CORE_STATUS_UNSUPPORTED, "double subscript", 3, 4},
    {"missing superscript argument",
     "x^",
     2,
     TEX_CORE_MODE_MATH_INLINE,
     TEX_CORE_STATUS_UNSUPPORTED,
     "missing superscript argument",
     1,
     2},
    {"missing subscript argument",
     "x_}",
     3,
     TEX_CORE_MODE_MATH_INLINE,
     TEX_CORE_STATUS_UNSUPPORTED,
     "missing subscript argument",
     2,
     3},
    {"superscript in document mode",
     "x^2",
     3,
     TEX_CORE_MODE_DOCUMENT,
     TEX_CORE_STATUS_UNSUPPORTED,
     "unsupported character U+005E",
     1,
     2},
    {"subscript in document mode",
     "x_2",
     3,
     TEX_CORE_MODE_DOCUMENT,
     TEX_CORE_STATUS_UNSUPPORTED,
     "unsupported character U+005F",
     1,
     2},
    {"alignment tab",
     "&",
     1,
     TEX_CORE_MODE_DOCUMENT,
     TEX_CORE_STATUS_UNSUPPORTED,
     "unsupported character U+0026",
     0,
     1},
    {"active tilde", "~", 1, TEX_CORE_MODE_DOCUMENT, TEX_CORE_STATUS_UNSUPPORTED, "unsupported character U+007E", 0, 1},
    {"math character outside the text surface",
     "a+b",
     3,
     TEX_CORE_MODE_DOCUMENT,
     TEX_CORE_STATUS_UNSUPPORTED,
     "unsupported character U+002B",
     1,
     2},
    {"unknown character in math mode",
     "a@b",
     3,
     TEX_CORE_MODE_MATH_INLINE,
     TEX_CORE_STATUS_UNSUPPORTED,
     "unsupported character U+0040",
     1,
     2},
    {"letter outside the embedded metrics",
     "\xC3\xA9",
     2,
     TEX_CORE_MODE_DOCUMENT,
     TEX_CORE_STATUS_UNSUPPORTED,
     "unsupported character U+00E9",
     0,
     2},

    {"unknown control word",
     "\\foo",
     4,
     TEX_CORE_MODE_MATH_INLINE,
     TEX_CORE_STATUS_UNSUPPORTED,
     "unsupported command \\foo",
     0,
     4},
    {"fraction command outside math mode",
     "\\frac{1}{2}",
     11,
     TEX_CORE_MODE_DOCUMENT,
     TEX_CORE_STATUS_UNSUPPORTED,
     "unsupported command \\frac",
     0,
     5},
    {"missing numerator at end of input",
     "\\frac",
     5,
     TEX_CORE_MODE_MATH_INLINE,
     TEX_CORE_STATUS_UNSUPPORTED,
     "missing numerator argument",
     0,
     5},
    {"missing denominator at end of input",
     "\\dfrac{1}",
     9,
     TEX_CORE_MODE_MATH_INLINE,
     TEX_CORE_STATUS_UNSUPPORTED,
     "missing denominator argument",
     0,
     6},
    {"script mark as a fraction argument",
     "\\frac^2 3",
     9,
     TEX_CORE_MODE_MATH_INLINE,
     TEX_CORE_STATUS_UNSUPPORTED,
     "missing numerator argument",
     5,
     6},
    {"spacing command as a fraction argument",
     "\\frac{1}\\quad",
     13,
     TEX_CORE_MODE_MATH_INLINE,
     TEX_CORE_STATUS_UNSUPPORTED,
     "missing denominator argument",
     8,
     13},
    {"fraction command as a fraction argument",
     "\\frac\\frac{1}{2}{3}",
     19,
     TEX_CORE_MODE_MATH_INLINE,
     TEX_CORE_STATUS_UNSUPPORTED,
     "missing numerator argument",
     5,
     10},
    {"closing brace over a pending denominator",
     "{\\tfrac{1}}",
     11,
     TEX_CORE_MODE_MATH_INLINE,
     TEX_CORE_STATUS_UNSUPPORTED,
     "missing denominator argument",
     10,
     11},
    {"missing right at end of input",
     "\\left(x",
     7,
     TEX_CORE_MODE_MATH_INLINE,
     TEX_CORE_STATUS_UNSUPPORTED,
     "missing \\right",
     0,
     5},
    {"missing right at a closing brace",
     "{\\left(x}",
     9,
     TEX_CORE_MODE_MATH_INLINE,
     TEX_CORE_STATUS_UNSUPPORTED,
     "missing \\right",
     8,
     9},
    {"unmatched right",
     "x\\right)",
     8,
     TEX_CORE_MODE_MATH_INLINE,
     TEX_CORE_STATUS_UNSUPPORTED,
     "unmatched \\right",
     1,
     7},
    {"missing delimiter after left",
     "\\left*x\\right)",
     15,
     TEX_CORE_MODE_MATH_INLINE,
     TEX_CORE_STATUS_UNSUPPORTED,
     "missing delimiter",
     5,
     6},
    {"missing delimiter at end of input",
     "x\\bigl",
     6,
     TEX_CORE_MODE_MATH_INLINE,
     TEX_CORE_STATUS_UNSUPPORTED,
     "missing delimiter",
     1,
     6},
    {"missing radical argument at end of input",
     "\\sqrt",
     5,
     TEX_CORE_MODE_MATH_INLINE,
     TEX_CORE_STATUS_UNSUPPORTED,
     "missing radical argument",
     0,
     5},
    {"script mark as a radical argument",
     "\\sqrt^2",
     7,
     TEX_CORE_MODE_MATH_INLINE,
     TEX_CORE_STATUS_UNSUPPORTED,
     "missing radical argument",
     5,
     6},
    {"unclosed radical index",
     "\\sqrt[3",
     7,
     TEX_CORE_MODE_MATH_INLINE,
     TEX_CORE_STATUS_UNSUPPORTED,
     "unclosed radical index",
     5,
     6},
    {"radical index closed by a brace",
     "{\\sqrt[3}",
     9,
     TEX_CORE_MODE_MATH_INLINE,
     TEX_CORE_STATUS_UNSUPPORTED,
     "unclosed radical index",
     8,
     9},
    {"misplaced limits",
     "x\\limits",
     8,
     TEX_CORE_MODE_MATH_INLINE,
     TEX_CORE_STATUS_UNSUPPORTED,
     "misplaced \\limits",
     1,
     8},
    {"misplaced nolimits after a non-operator",
     "{a}\\nolimits",
     12,
     TEX_CORE_MODE_MATH_INLINE,
     TEX_CORE_STATUS_UNSUPPORTED,
     "misplaced \\nolimits",
     3,
     12},
    {"missing accent argument at end of input",
     "\\hat",
     4,
     TEX_CORE_MODE_MATH_INLINE,
     TEX_CORE_STATUS_UNSUPPORTED,
     "missing accent argument",
     0,
     4},
    {"script mark as an accent argument",
     "\\vec^2",
     6,
     TEX_CORE_MODE_MATH_INLINE,
     TEX_CORE_STATUS_UNSUPPORTED,
     "missing accent argument",
     4,
     5},
    {"accent command as an accent argument",
     "\\hat\\bar{x}",
     11,
     TEX_CORE_MODE_MATH_INLINE,
     TEX_CORE_STATUS_UNSUPPORTED,
     "missing accent argument",
     4,
     8},
    {"missing style argument at end of input",
     "\\mathbf",
     7,
     TEX_CORE_MODE_MATH_INLINE,
     TEX_CORE_STATUS_UNSUPPORTED,
     "missing style argument",
     0,
     7},
    {"missing text argument at end of input",
     "\\text",
     5,
     TEX_CORE_MODE_MATH_INLINE,
     TEX_CORE_STATUS_UNSUPPORTED,
     "missing text argument",
     0,
     5},
    {"calligraphic lowercase is outside the face",
     "\\mathcal{a}",
     11,
     TEX_CORE_MODE_MATH_INLINE,
     TEX_CORE_STATUS_UNSUPPORTED,
     "unsupported character U+0061",
     9,
     10},
    {"math surface stays out of text content",
     "\\text{a+b}",
     10,
     TEX_CORE_MODE_MATH_INLINE,
     TEX_CORE_STATUS_UNSUPPORTED,
     "unsupported character U+002B",
     7,
     8},
    {"left as a script argument",
     "x^\\left(y\\right)",
     17,
     TEX_CORE_MODE_MATH_INLINE,
     TEX_CORE_STATUS_UNSUPPORTED,
     "missing superscript argument",
     2,
     7},
    {"math symbol command outside math mode",
     "\\alpha",
     6,
     TEX_CORE_MODE_DOCUMENT,
     TEX_CORE_STATUS_UNSUPPORTED,
     "unsupported command \\alpha",
     0,
     6},
    {"unknown control word in document mode",
     "a \\usepackage",
     13,
     TEX_CORE_MODE_DOCUMENT,
     TEX_CORE_STATUS_UNSUPPORTED,
     "unsupported command \\usepackage",
     2,
     13},
    {"unknown control symbol",
     "\\@",
     2,
     TEX_CORE_MODE_DOCUMENT,
     TEX_CORE_STATUS_UNSUPPORTED,
     "unsupported command \\@",
     0,
     2},
    {"incomplete control sequence",
     "ab\\",
     3,
     TEX_CORE_MODE_DOCUMENT,
     TEX_CORE_STATUS_UNSUPPORTED,
     "incomplete control sequence at end of input",
     2,
     3},

    {"paragraph break",
     "a\n\nb",
     4,
     TEX_CORE_MODE_DOCUMENT,
     TEX_CORE_STATUS_UNSUPPORTED,
     "unsupported paragraph break",
     1,
     3},
    {"paragraph break with blank-line whitespace",
     "a\n \t\nb",
     6,
     TEX_CORE_MODE_DOCUMENT,
     TEX_CORE_STATUS_UNSUPPORTED,
     "unsupported paragraph break",
     1,
     5},
    {"paragraph break after a control word",
     "\\quad\n\nx",
     8,
     TEX_CORE_MODE_DOCUMENT,
     TEX_CORE_STATUS_UNSUPPORTED,
     "unsupported paragraph break",
     5,
     7},
    {"paragraph break with spaces after a control word",
     "\\quad \n \nx",
     10,
     TEX_CORE_MODE_DOCUMENT,
     TEX_CORE_STATUS_UNSUPPORTED,
     "unsupported paragraph break",
     5,
     9},

    {"unknown environment",
     "\\begin{foo}a\\end{foo}",
     21,
     TEX_CORE_MODE_MATH_INLINE,
     TEX_CORE_STATUS_UNSUPPORTED,
     "unsupported environment foo",
     0,
     11},
    {"environment name without a brace",
     "\\begin x",
     8,
     TEX_CORE_MODE_MATH_INLINE,
     TEX_CORE_STATUS_UNSUPPORTED,
     "missing environment name",
     7,
     8},
    {"environment name at end of input",
     "\\begin",
     6,
     TEX_CORE_MODE_MATH_INLINE,
     TEX_CORE_STATUS_UNSUPPORTED,
     "missing environment name",
     0,
     6},
    {"environment name left unclosed",
     "\\begin{matrix",
     13,
     TEX_CORE_MODE_MATH_INLINE,
     TEX_CORE_STATUS_UNSUPPORTED,
     "missing environment name",
     0,
     6},
    {"empty environment name",
     "\\begin{}",
     8,
     TEX_CORE_MODE_MATH_INLINE,
     TEX_CORE_STATUS_UNSUPPORTED,
     "missing environment name",
     7,
     8},
    {"mismatched environment end",
     "\\begin{matrix}a\\end{pmatrix}",
     28,
     TEX_CORE_MODE_MATH_INLINE,
     TEX_CORE_STATUS_UNSUPPORTED,
     "mismatched \\end{pmatrix}",
     15,
     28},
    {"unmatched environment end",
     "a\\end{matrix}",
     13,
     TEX_CORE_MODE_MATH_INLINE,
     TEX_CORE_STATUS_UNSUPPORTED,
     "unmatched \\end{matrix}",
     1,
     13},
    {"environment end inside a group",
     "\\begin{matrix}{a\\end{matrix}",
     28,
     TEX_CORE_MODE_MATH_INLINE,
     TEX_CORE_STATUS_UNSUPPORTED,
     "unmatched \\end{matrix}",
     16,
     28},
    {"environment left unclosed at end of input",
     "\\begin{matrix}a",
     15,
     TEX_CORE_MODE_MATH_INLINE,
     TEX_CORE_STATUS_UNSUPPORTED,
     "missing \\end{matrix}",
     0,
     14},
    {"closing brace inside an environment cell",
     "\\begin{matrix}a}",
     16,
     TEX_CORE_MODE_MATH_INLINE,
     TEX_CORE_STATUS_UNSUPPORTED,
     "missing \\end{matrix}",
     15,
     16},
    {"alignment tab outside an environment",
     "a&b",
     3,
     TEX_CORE_MODE_MATH_INLINE,
     TEX_CORE_STATUS_UNSUPPORTED,
     "misplaced alignment tab",
     1,
     2},
    {"alignment tab inside a cell's group",
     "\\begin{matrix}{a&b}\\end{matrix}",
     31,
     TEX_CORE_MODE_MATH_INLINE,
     TEX_CORE_STATUS_UNSUPPORTED,
     "misplaced alignment tab",
     16,
     17},
    {"row terminator outside an environment",
     "a\\\\b",
     4,
     TEX_CORE_MODE_MATH_INLINE,
     TEX_CORE_STATUS_UNSUPPORTED,
     "misplaced \\\\",
     1,
     3},
    {"alignment tab in document mode stays reserved",
     "a&b",
     3,
     TEX_CORE_MODE_DOCUMENT,
     TEX_CORE_STATUS_UNSUPPORTED,
     "unsupported character U+0026",
     1,
     2},
    {"environment in document mode",
     "\\begin{matrix}a\\end{matrix}",
     27,
     TEX_CORE_MODE_DOCUMENT,
     TEX_CORE_STATUS_UNSUPPORTED,
     "unsupported command \\begin",
     0,
     6},
    {"environment as a script argument",
     "x^\\begin{matrix}a\\end{matrix}",
     29,
     TEX_CORE_MODE_MATH_INLINE,
     TEX_CORE_STATUS_UNSUPPORTED,
     "missing superscript argument",
     2,
     8},
    {"environment as a fraction argument",
     "\\frac\\begin{matrix}1",
     20,
     TEX_CORE_MODE_MATH_INLINE,
     TEX_CORE_STATUS_UNSUPPORTED,
     "missing numerator argument",
     5,
     11},
    {"pending script at an alignment tab",
     "\\begin{matrix}a^&b\\end{matrix}",
     30,
     TEX_CORE_MODE_MATH_INLINE,
     TEX_CORE_STATUS_UNSUPPORTED,
     "missing superscript argument",
     16,
     17},
    {"pending script at a row terminator",
     "\\begin{matrix}a_\\\\b\\end{matrix}",
     31,
     TEX_CORE_MODE_MATH_INLINE,
     TEX_CORE_STATUS_UNSUPPORTED,
     "missing subscript argument",
     16,
     18},
    {"optional row spacing after a row terminator",
     "\\begin{matrix}a\\\\[4pt]b\\end{matrix}",
     35,
     TEX_CORE_MODE_MATH_INLINE,
     TEX_CORE_STATUS_UNSUPPORTED,
     "unsupported row spacing",
     17,
     18},
    {"optional row spacing behind blanks and a star",
     "\\begin{matrix}a\\\\* [4pt]b\\end{matrix}",
     37,
     TEX_CORE_MODE_MATH_INLINE,
     TEX_CORE_STATUS_UNSUPPORTED,
     "unsupported row spacing",
     19,
     20},
    {"eleventh matrix column",
     "\\begin{matrix}1&2&3&4&5&6&7&8&9&0&9\\end{matrix}",
     47,
     TEX_CORE_MODE_MATH_INLINE,
     TEX_CORE_STATUS_UNSUPPORTED,
     "extra alignment tab",
     33,
     34},
    {"starred environment name reports itself",
     "\\begin{align*}x\\end{align*}",
     27,
     TEX_CORE_MODE_MATH_INLINE,
     TEX_CORE_STATUS_UNSUPPORTED,
     "unsupported environment align*",
     0,
     14},
    {"third cases column",
     "\\begin{cases}a&b&c\\end{cases}",
     29,
     TEX_CORE_MODE_MATH_INLINE,
     TEX_CORE_STATUS_UNSUPPORTED,
     "extra alignment tab",
     16,
     17},
};

int main(void) {
    txc_test test = {0, 0};
    for (size_t index = 0; index < sizeof(TXC_ERROR_CASES) / sizeof(TXC_ERROR_CASES[0]); index++) {
        const txc_error_case *expected = &TXC_ERROR_CASES[index];
        tex_core_options options;
        tex_core_options_init(&options);
        options.mode = expected->mode;
        tex_core_render_tree *tree = NULL;
        tex_core_error error;
        tex_core_status status =
            tex_core_document_compile((const uint8_t *)expected->source, expected->length, &options, &tree, &error);

        txc_check(
            &test,
            status == expected->status,
            "%s: status %d, expected %d",
            expected->label,
            (int)status,
            (int)expected->status
        );
        txc_check(&test, tree == NULL, "%s: tree stays NULL", expected->label);
        txc_check(&test, error.status == status, "%s: error mirrors the status", expected->label);
        txc_check_text(&test, error.message, expected->message, expected->label);
        txc_check(&test, error.has_range, "%s: error carries a range", expected->label);
        txc_check_size(&test, error.range.begin, expected->begin, expected->label);
        txc_check_size(&test, error.range.end, expected->end, expected->label);
    }
    return txc_test_finish(&test, "errors");
}
