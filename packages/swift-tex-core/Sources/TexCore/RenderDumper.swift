...
struct RenderDumper: RenderVisitor {
    private(set) var output = "render-tree 3\n"
    private var depth = 0
    ...
    mutating func visit(_ node: Rule) {
        indent()
        output += "rule x=\(measure(node.x)) y=\(measure(node.y)) width=\(measure(node.width))"
        output += " ascent=\(measure(node.ascent)) descent=\(measure(node.descent)) src=\(range(node.src))\n"
    }
