...
# Decide whether there is anything to format before touching the tool
# bootstrap: a no-op run must not require Python or the network, and must
# not mutate .tools/. The venv install lives in
# scripts/init-environment.sh --install tools.
